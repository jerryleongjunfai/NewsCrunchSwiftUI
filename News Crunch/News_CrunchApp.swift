...
class AppDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any]? = nil) -> Bool {
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge]) { success, error in
            print(success)
            print(error?.localizedDescription ?? "No error for authorised permission")
        }
        return true
    }
...
struct News_CrunchApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView()
                ...
                .onReceive(NotificationCenter.default.publisher(for: .articleSent), perform: handleOnRecieveNotification)
    ...
    private func handleOnRecieveNotification(_ notification: Notification) {
